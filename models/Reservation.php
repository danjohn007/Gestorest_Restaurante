...
class Reservation extends BaseModel {
    ...
    /**
     * Check for reservations within a time buffer for a specific table
     * @param int $tableId Table ID to check
     * @param DateTime $now Current datetime
     * @param DateTime $bufferTime Datetime with buffer added
     * @return array|false Reservation data or false if none found
     */
    public function getReservationWithinTimeBuffer($tableId, $now, $bufferTime) {
        $query = "SELECT r.*, rt.table_id 
                  FROM reservations r 
                  JOIN reservation_tables rt ON r.id = rt.reservation_id 
                  WHERE rt.table_id = ? 
                  AND r.status IN ('pendiente', 'confirmada') 
                  AND r.reservation_datetime BETWEEN ? AND ?";
        
        $stmt = $this->db->prepare($query);
        $stmt->execute([
            $tableId,
            $now->format('Y-m-d H:i:s'),
            $bufferTime->format('Y-m-d H:i:s')
        ]);
