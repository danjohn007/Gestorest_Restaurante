...
<?php elseif ($user['role'] === ROLE_WAITER): ?>
    ...
    <!-- Expired Orders Alert for Waiters (if any) -->
    <?php if (isset($expired_orders_count) && $expired_orders_count > 0): ?>
    <div class="row mb-4">
        <div class="col-12">
            <div class="alert alert-warning border-0 shadow-sm" role="alert">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-grow-1 ms-3">
                        <h5 class="alert-heading mb-1">¡Atención! Tienes Pedidos Vencidos</h5>
                        <p class="mb-2">
                            Tienes <strong><?= $expired_orders_count ?></strong> pedidos de días anteriores que aún no han sido entregados.
                        </p>
                        <a href="<?= BASE_URL ?>/orders/expiredOrders" class="btn btn-warning btn-sm">
                            <i class="bi bi-arrow-right-circle"></i> Ver Mis Pedidos Vencidos
                        </a>
                    </div>
                </div>
            </div>
    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="card border-success shadow-lg" style="background: linear-gradient(135deg, #28a745 0%, #20c997 100%);">
                <div class="card-body py-4">
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            <div class="d-flex align-items-center">
                                <div class="me-4">
                                    <i class="bi bi-plus-circle-fill text-white" style="font-size: 3.5rem;"></i>
                                </div>
                                <div>
                                    <h3 class="text-white mb-1 fw-bold">¡Crear Nuevo Pedido!</h3>
                                    <p class="text-white-50 mb-0 fs-6">
                                        Toma órdenes rápidamente y gestiona tus mesas de manera eficiente
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4 text-end">
                            <a href="<?= BASE_URL ?>/orders/create" class="btn btn-light btn-lg px-4 py-3 shadow-sm">
    ...
    <div class="row mt-4">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">
                        <i class="bi bi-table"></i> Todas las Mesas (No Asignadas)
                    </h5>
                    <small class="text-muted">Mesas que no están asignadas o están ocupadas por otros meseros</small>
                </div>
                <div class="card-body">
                    <?php if (!empty($unassigned_tables)): ?>
                        <div class="row">
                            <?php foreach ($unassigned_tables as $table): ?>
                            <div class="col-md-2 mb-3">
                                <div class="card table-card <?= $table['status'] ?>" style="cursor: default;">
                                    <div class="card-body text-center p-3">
                                        <h4 class="mb-1">Mesa <?= $table['number'] ?></h4>
                                        <span class="badge table-status status-<?= $table['status'] ?>">
                                            <?= getStatusText($table['status']) ?>
                                        </span>
                                        <?php if ($table['current_waiter_name']): ?>
                                            <div class="mt-2">
                                                <small class="text-primary"><strong>Mesero:</strong> <?= htmlspecialchars($table['current_waiter_name']) ?></small>
                                            </div>
                                        <?php else: ?>
                                            <div class="mt-2">
                    ...
                    <?php else: ?>
                        <p class="text-muted mb-0">Todas las mesas están asignadas a meseros.</p>
                    <?php endif; ?>
                </div>
            </div>
        </div>
