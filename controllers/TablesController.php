...
class TablesController extends BaseController {
    ...
    public function index() {
        ...
        // For waiters, show enhanced table information including assignments
        if ($user['role'] === ROLE_WAITER) {
            // Get tables with detailed waiter and occupancy information
            $query = "SELECT t.*, w.employee_code, u.name as waiter_name,
                             CASE 
                                 WHEN t.waiter_id IS NULL THEN 'Sin asignar'
                                 WHEN t.waiter_id = ? THEN 'Asignada a ti'
                                 ELSE CONCAT('Asignada a ', u.name)
                             END as assignment_status,
                             CASE 
                                 WHEN t.waiter_id IS NULL THEN 'available'
                                 WHEN t.waiter_id = ? THEN 'mine'
                                 ELSE 'assigned'
            ...
            if ($search) {
                $query .= " AND (t.number LIKE ? OR u.name LIKE ? OR w.employee_code LIKE ?)";
                $params[] = "%{$search}%";
                $params[] = "%{$search}%";
                $params[] = "%{$search}%";
            }
            ...
            // Get total count - build separate count query to avoid parameter mismatch
            ...
            if ($search) {
                $countQuery .= " AND (t.number LIKE ? OR u.name LIKE ? OR w.employee_code LIKE ?)";
                $countParams[] = "%{$search}%";
                $countParams[] = "%{$search}%";
                $countParams[] = "%{$search}%";
            }
    ...
    private function validateTableInput($data, $excludeId = null) {
        ...
        // Validate table identifier format (letter optional + numbers)
        if (!empty($number) && !preg_match('/^[A-Za-z]?[0-9]+$/', $number)) {
            $errors['number'] = 'El identificador debe ser una letra opcional seguida de números (Ej: A1, B2, 1, 2)';
        }
        
        if (empty($number)) {
            $errors['number'] = 'El identificador de mesa es obligatorio';
        }
        
        if (strlen($number) > 10) {
            $errors['number'] = 'El identificador no puede tener más de 10 caracteres';
        }
        ...
        if (!empty($number) && $this->tableModel->numberExists($number, $excludeId)) {
            $errors['number'] = 'Ya existe una mesa con este identificador';
        }
    ...
    public function liberateAllTables() {
        $this->requireRole([ROLE_ADMIN]);
        
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                // Check if all daily orders are delivered
                $orderModel = new Order();
                $pendingOrders = $orderModel->getPendingOrdersForToday();
                
                if (!empty($pendingOrders)) {
                    $this->redirect('tables', 'error', 'No se pueden liberar las mesas. Hay ' . count($pendingOrders) . ' pedidos pendientes de entrega del día de hoy.');
                    return;
                }
                
                // All orders are delivered, liberate all tables
                $this->tableModel->liberateTablesDaily();
                
                $this->redirect('tables', 'success', 'Todas las mesas han sido liberadas correctamente.');
                
            } catch (Exception $e) {
                $this->redirect('tables', 'error', 'Error al liberar las mesas: ' . $e->getMessage());
            }
        } else {
            // Show confirmation page
            $orderModel = new Order();
            $pendingOrders = $orderModel->getPendingOrdersForToday();
            $totalTables = $this->tableModel->count(['active' => 1]);
            
            $this->view('tables/liberate_confirmation', [
